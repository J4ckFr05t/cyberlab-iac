...
locals {
  ...
  # Tier 2: 2+ dependencies (Unix/Linux Workloads)
  vms_tier_2 = {
    for k, v in local.vms : k => v if length(try(v.depends_on, [])) >= 2 && !can(regex("win", v.clone))
  }

  # Tier 3: Windows Workloads (Sequential to Tier 2 to save I/O)
  vms_tier_3 = {
    for k, v in local.vms : k => v if length(try(v.depends_on, [])) >= 2 && can(regex("win", v.clone))
  }
}
...
# --- TIER 3 RESOURCES (Depend on Tier 2) ---
resource "proxmox_vm_qemu" "tier_3" {
  for_each    = local.vms_tier_3
  name        = each.value.name
  target_node = each.value.target_node
  vmid        = each.value.vmid
  clone       = each.value.clone
  ...
  depends_on = [proxmox_vm_qemu.tier_2]
}
